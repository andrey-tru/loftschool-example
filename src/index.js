...
function delegate(target, fn) {
  target.addEventListener('click', function(result) {
    if (result.target.tagName == 'BUTTON') {
      fn();
    }
  });
}
...
function once(target, fn) {
  target.addEventListener('click', function cl() {
    fn();
    target.removeEventListener('click', cl);
  });
}
